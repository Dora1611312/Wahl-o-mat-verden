...
// ========================================
// JSON laden – PFAD: data/Buergermeister.json
// ========================================

fetch("./data/Buergermeister.json")
    .then(response => {
        if (!response.ok) {
    ...
    .then(json => {
        data = json;
        console.log("✅ Bürgermeister-Wahl-O-Mat Daten geladen:", data);
        console.log(`📋 ${data.thesen.length} Thesen geladen.`);
    })
    .catch(error => {
        console.error("❌ Fehler beim Laden der JSON:", error);
        alert("Die Wahldaten konnten nicht geladen werden.\nBitte prüfe, ob die Datei 'data/Buergermeister.json' existiert und gültig ist.");
    });
...
function showResults() {
    ...
    console.log("📊 Wahlergebnis (Bürgermeister):", results);
}
...
console.log("🗳️ Bürgermeister-Wahl-O-Mat Verden 2026 geladen.");